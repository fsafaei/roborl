...
| `diagnostics/her_virtual_reward_zero_fraction` | HER: share of relabeled rewards equal to 0 (goal hit under the substitute goal) | Task-shaped. Push: starts ≈ 0.98 while the box is never touched (achieved goal constant within an episode, so every future goal is trivially hit), then *falls* as the policy starts moving the box (0.88 by 60k in the pilot); Reach: 0.7–0.8 throughout (slow gripper, most future goals within 5 cm). The analytic floor from own-successor hits alone is `H_T/T` ≈ 0.09 at T = 50 | ≈ 0 → the reward is recomputed on the wrong achieved goal (pre-step instead of post-step) or the future-index math excludes the own successor; *stuck* ≈ 1 past ~100k on Push → the policy never interacts with the object |
