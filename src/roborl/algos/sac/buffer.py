"""Uniform replay buffer, local to SAC (rule of three — ADR 0003).

A preallocated NumPy ring buffer instead of a framework dependency: the
buffer is itself curriculum. The two details that matter live here:
...
- **The stored done flag is ``terminated`` only.** Targets bootstrap through
  ...
  means a non-autoresetting env (see ``sac.py``), because an autoreset
